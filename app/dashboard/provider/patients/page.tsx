...
export default function PatientManagementPage() {
  ...
  // Handler untuk Add New Patient
  const handleAddNewPatient = () => {
    router.push("/dashboard/provider/patients/new")
  }

  // Handler untuk View Details
  const handleViewDetails = (id: number) => {
    router.push(`/dashboard/provider/patients/${id}`)
  }

  // Handler untuk Edit Patient
  const handleEditPatient = (id: number) => {
    router.push(`/dashboard/provider/patients/${id}/edit`)
  }

  // Handler untuk Schedule Appointment
  const handleScheduleAppointment = (id: number) => {
    router.push(`/appointments/schedule?patientId=${id}`)
  }

  // Handler untuk Send Message
  const handleSendMessage = (id: number) => {
    router.push(`/messages?patientId=${id}`)
  }

  // Handler untuk Call Patient
  const handleCallPatient = (id: number) => {
    router.push(`/video-call?patientId=${id}`)
  }

  // Handler untuk OpenPose Analysis
  const handleOpenPose = (id: number) => {
    router.push(`/pose-estimation?patientId=${id}`)
  }

  // Handler untuk Delete Patient
  ...
  // Handler untuk Filter & Export (placeholder)
  const handleFilter = () => {
      ...
      <div className="flex-1 overflow-auto">
        <div className="p-8">
          ...
          <div className="flex justify-between items-center mb-8">
            ...
            <Button className="bg-[#014585] hover:bg-[#013a70]" onClick={handleAddNewPatient}>
                  ...
                  filteredPatients.map((patient) => (
                    <div key={patient.id} className="grid grid-cols-12 gap-2 px-6 py-4 hover:bg-gray-50">
                      ...
                      <div className="col-span-2 flex items-center space-x-2">
                        <Button size="sm" className="bg-[#014585] hover:bg-[#013a70]" onClick={() => handleViewDetails(patient.id)}>
                          View Details
                        </Button>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem onClick={() => handleSendMessage(patient.id)}>
                              <Mail className="mr-2 h-4 w-4" /> Send Message
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => handleCallPatient(patient.id)}>
                              <Phone className="mr-2 h-4 w-4" /> Call Patient
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => handleOpenPose(patient.id)}>
